...
for (const file of commandFiles) {
  ...
  command.aliases
    .split(",")
    .forEach((al) => commandAliases.push({ alias: al, cmd: command.name }))
}
...
bot.on("message", async (message) => {
  ...
  if (
    bot.commands.get(command).channels &&
    !bot.commands.get(command).channels.includes(message.channel.id)
  )
